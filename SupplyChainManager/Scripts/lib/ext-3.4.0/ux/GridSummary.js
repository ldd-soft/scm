...
Ext.extend(Ext.ux.grid.GridSummary, Ext.util.Observable, {
    ...
    calculate: function(rs, cm) {
        ...
        for (var i = 0, len = cfg.length; i < len; i++) { // loop through all columns in ColumnModel
            ...
            if (cf.totalSummaryType) {
                
                if(cf.totalSummaryType == 'last') {
                    if(rs.length > 0) {
                        var r = rs[rs.length-1]; // get the last Record
                        data[cname] = r.get(cname);
                    }
                }
                else if(cf.totalSummaryType == 'first') {
                    if(rs.length > 0) {
                        var r = rs[0]; // get the first Record
                        data[cname] = r.get(cname);
                    }
                }
                else {
    ...
    renderSummary: function(o, cs, cm) {
        ...
        for (var i = 0, len = cs.length; i < len; i++) {
            ...
            if (cf.totalSummaryTitle) {
                p.value = cf.totalSummaryTitle;
            }
