...
var buildGrid = function () {
    ...
    var expander = new Ext.ux.grid.RowPanelExpander({
        ...
        createExpandingRowPanelItems: function (rec1, rowIndex) {
            ...
            ds_item.baseParams.exit_id = rec1.data.Id;
            ds_item.reload();
    ...
    var toolbar = new Ext.Toolbar({
        cls: 'headtoolbar',
        items: ['<img class="HeadingColorTag2" border="0" src="' + root_path + 'content/Images/HeadingColorTag_Theme01.png"/> <div class="ThemeHeadingText" id="list_title">出库列表 : </div>'
            , '->'
            , '查看列表: '
            ...
            listeners: {
                'select': function (combo, record, index) {
                    ...
                    if (index == 1) {
                        ...
                        grid.getStore().reload();
                        //Ext.fly('list_title').update('出库列表 : ');
                    }
                }
            }
        })
    ...
    var columns_item = [
                ...
                { header: '应发数量', width: 60, dataIndex: 'QuantityNeed' },
                { header: '实发数量', width: 60, dataIndex: 'QuantityReal' },
...
var newFn = function () {
    ...
    loadModule('AddExit', '添加出库', 'store/exit/add');
};

var editFn = function (rec) {
    parentId = 'Exit';
    currentRec = rec;
    loadModule('EditExit', '编辑出库', 'store/exit/edit');
};
